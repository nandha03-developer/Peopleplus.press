...
export async function generateMetadata({ params }: any): Promise<Metadata> {
  //const { metrodetailsid} = params;
  const metrodetailsid = (await params).metrodetailsid
  try {
    const groupResponse = await fetch(`https://s1jota6yr2.execute-api.ap-south-1.amazonaws.com/default/List_api_tables?table_name=News&url_contains=${metrodetailsid}`);
   
    const groupData: any = await groupResponse.json();
    const newsData = groupData.Data.find((post: any) => post.url === metrodetailsid);
