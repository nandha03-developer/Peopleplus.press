...
const Page = ({ params }: any) => {
  ...
  const formattedState = formatStateId(resolvedParams.districtId);
  const stateParams = resolvedParams.stateId
  const districtParams = resolvedParams.districtId
  ...
  const { fetchNewsView, error, newsData } = useNewsContext(); //Most view
  ...
  const fetchDataLocation = async (stateNameFromURL: string) => {
    try {
      ...
      const normalizedStateNameFromURL = stateNameFromURL.trim().toLowerCase();
      const districtData = data.find((row: any) => row.district_name.trim().toLowerCase() === normalizedStateNameFromURL);
      if (districtData) {
  ...
  const fetchDataFromApi = async (districtId: any) => {
    ...
    try {
      const response = await axios.get(`/List_api_tables?table_name=News&stateid_eq=${stateId}&districtid_eq=${districtId}&language_eq=${languageId}&limit=9&groupid_eq=23`);
      ...
      const mostView = await axios.get(`/List_api_tables?table_name=News&stateid_eq=${stateId}&districtid_eq=${districtId}&limit=5&offset=0&sort_by=Views&order=Desc&language_contains=${languageId}&groupid_eq=23`);
      const mostViewedNews = mostView.data.Data
      setMostViewed(mostViewedNews);
      //To fetch most popular news
      const popular = await axios.get(`/Popularnews?stateid=${stateId}&districtid=${districtId}&limit=5&language=${language}&groupid=23`);
      const popularNews = popular.data
      setPopularNews(popularNews);

      //To fetch short news
      const responseshorts = await axios.get(
        `/List_api_tables?table_name=News&groupid_eq=23&stateid_eq=${stateId}&districtid_eq=${districtId}&language_contains=${languageId}&shorts_contains=.`
      );
  ...
  const handleClick = (item: any) => {
    ...
    router.push(`/${lang}/india/${stateParams}/${districtParams}/${item.url}`);
  };
  ...
  const fetchNewsData = async (page: any, stateId: any, districtId: any) => {
    ...
    try {
      const latestNewsRes = await axios.get(`/List_api_tables?table_name=News&limit=${itemsPerPage}&status_eq=true&offset=${offset}&language_contains=${languageCode}&stateid_eq=${stateId}&districtid_eq=${districtId}&groupid_eq=23`);
  ...
  useEffect(() => {
    if(!districtId || districtId == undefined) return
    fetchNewsData(currentPage, stateId, districtId);
  }, [currentPage, stateId, districtId]);
              ...
              <div className="slider-containerss">
                <div className="slider-contentss">
                  {trending.map((item: any, index: any) => (
                    <div key={item.id} onClick={() => handleClick(item)} className="item" style={{ transform: `translateY(-${index * 100}%)`, transition: 'transform 4s ease', opacity: index === currentIndex ? 1 : 0, height: '100%', }}>
                      <Link href={`/${lang}/india/${stateParams}/${districtParams}/${item.url || item.title}`}>{item.title}</Link>
                    </div>
                  ))}
                </div>
                ...
                <div className="col">
                  {isFallback ? (
                    <h1 className="mb-sm-0">
                      <strong>Related News in India</strong>
                    </h1>
                  ) : (
                    <h1 className="mb-sm-0">
                ...
                <div className="col-12 col-sm-auto">
                  <nav aria-label="breadcrumb">
                    <ol className="breadcrumb d-inline-block">
                      ...
                      <li className="breadcrumb-item" aria-current="page">
                        <Link href={`/${lang}/india/${stateParams}`}>{stateName}</Link>
                      </li>
                            ...
                            <div key={post.id} className="col-6 col-sm-6 thm-padding">
                              <div className="slider-post post-height-2" >
                                ...
                                <div className="post-text">
                                  <span className="post-category" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleTags(post.tags, post) }} style={{ cursor: 'pointer', marginTop: '5px', pointerEvents: "auto", }}>{post.tags}</span>
                                  {loadingSkeleton ? (
                          ...
                          {latestNews.map((item: any) => (
                            ...
                            >
                              <div className="img-wrapper" style={{ cursor: "pointer" }}>
                                ...
                                {/* <Link href={`/${lang}/india/${params.stateId}/${params.districtId}/${item.url || item.title}`} className="thumb"> */}
                              ...
                              <div className="post-info-2">
                                {loadingSkeleton ? (
                                  ...
                                  <h4>
                                    <div onClick={() => handleClick(item)} style={{ cursor: "pointer" }} className="title">{item.title}</div></h4>
                                )}
                                <ul className="authar-info d-flex flex-wrap">
                                  <li><span className="post-category" style={{ cursor: 'pointer', marginTop: '5px', pointerEvents: "auto", }} onClick={() => handleTags(item.tags, item)}>{item.tags}</span></li>
