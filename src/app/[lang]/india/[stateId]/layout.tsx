...
export async function generateMetadata({ params }: any): Promise<Metadata> {
  ...
  const stateId = (await params).stateId
   const formattedState = formatStateId(stateId);
  try {
    const groupResponse = await fetch(`https://s1jota6yr2.execute-api.ap-south-1.amazonaws.com/default/List_api_tables?table_name=State`);
    const groupData: any = await groupResponse.json();
    const states = groupData.Data
    const groups = states.find((state: any) => state.state == formattedState)
