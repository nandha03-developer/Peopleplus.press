...
const Page = ({ params }: any) => {
  const { webStoryId }: any = use(params);
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      ...
      const newsDetail = await axios.get(
        `/List_api_tables?table_name=News&status_eq=true&language_contains=${languageId}&url_contains=${webStoryId}&shorts_contains=.`
      );
      const foundDetail = newsDetail.data.Data.find(
        (item: any) => item.url == webStoryId
      );
  ...
  return (
    <div>
      <Head>
        ...
        <Webstories webStoryUrl={webStoryId} shareUrl={shareUrl}         
